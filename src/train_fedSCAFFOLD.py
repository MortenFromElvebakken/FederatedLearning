...
from src.fl_standalone.fed_algs.model_trainer_fedScaffold import MyModelTrainer
...
parser.add_argument('--algorithm', default='fedSCAFFOLD', type=str, help='Which federated learning training alg to use')
...
for client in range(args.clients):
    client_control_variate.append([torch.zeros_like(p.data) for p in model.parameters() if p.requires_grad])

server_control_variate = [torch.zeros_like(p.data) for p in model.parameters() if p.requires_grad]
delta_control_variate = [torch.zeros_like(p.data) for p in model.parameters() if p.requires_grad]
